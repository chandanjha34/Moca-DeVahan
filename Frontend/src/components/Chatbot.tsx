...
const handleSend = async (e: React.FormEvent) => {
  ...
  try {
    if (mode === 'old' && activeCar) {
      ...
      const hashRes = await fetch("https://moca-devahan.onrender.com/addService/display", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const recordPromises = hashData.serviceHashes.map(async (hash: string) => {
        try {
          const res = await fetch(`https://moca-devahan.onrender.com/api/fetchJSON/${hash}`);
      ...
      const aiRes = await fetch("https://moca-devahan.onrender.com/api/ask0GCompute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tokenID: tokenId,
